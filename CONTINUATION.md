...
1. **DROP THE LOGO FILE.** Save the provided Scribe logo as
   `src/scribe/ui/assets/scribe.png` (square PNG; a `scribe.ico` too for the
   installer/exe icon). Everything already points at it. Until then the app
   shows the teal-mic fallback. Also wire `scribe.ico` into `scribe.spec`
   (EXE `icon=`) and `installer.iss` (`SetupIconFile`).
2. **Port the first-run wizard to QML** (`ui/qml/Wizard*.qml`) to match the dark
   theme, and show it from `qml_app.run_qml_ui` when no config file exists
   (currently the QML path SKIPS onboarding — a fresh install gets the window
   but no mic/hotkey/model-download wizard). The old Qt-Widgets wizard still
   exists in `ui/wizard.py` for reference.
...
4. **x64 + Linux builds**; **code-signing** (kills SmartScreen warning).
