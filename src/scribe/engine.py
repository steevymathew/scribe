...
from .hotkeys import key_name, match_key, vk_of
...
class Scribe:
    ...
    def on_press(self, key):
        if self.paused:
            return
        if self.debug:
            # The virtual key code is included because on Windows it is often the
            # only thing that identifies the key: the name can arrive as a bare
            # KeyCode with no name at all. See scribe.hotkeys.match_key.
            _out(f"  [DBG] press: {key!r}  vk={vk_of(key)}  boost_held={self.boost_held}")
    ...
    def on_release(self, key):
        if self.paused:
            ...
            return
        if self.debug:
            _out(f"  [DBG] release: {key!r}  vk={vk_of(key)}  boost_held={self.boost_held}")
        if match_key(key, self.boost_key):
            self._set_boost(False)
        if match_key(key, self.hotkey):
            self.stop_recording()
